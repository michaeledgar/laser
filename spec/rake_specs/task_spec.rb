...
describe Rake::LaserTask do
  describe '#initialize' do
    it 'yields to allow setting :libs and :extras' do
      ...
      task = Rake::LaserTask.new(task_name) do |wool|
        wool.libs = "LOL"
        wool.extras = "hai"
      end
  ...
  describe '#run' do
    it 'searches the listed libraries for files' do
      ...
      task = Rake::LaserTask.new("temptask3-#{rand(65329)}".to_sym) do |wool|
        wool.libs << 'lib' << 'spec'
      end
    ...
    it 'scans the matching files' do
      ...
      Dir.should_receive(:[]).with('spec/**/*.rb').and_return([])
      task = Rake::LaserTask.new("temptask4-#{rand(65329)}".to_sym) do |wool|
        wool.libs << 'lib' << 'spec'
      end
