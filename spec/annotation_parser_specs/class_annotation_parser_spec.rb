...
describe Parsers::ClassParser do
  ...
  describe 'a simple class name' do
    it 'is parsed into a single covariant constraint' do
      'Hello'.should parse_to([Types::ClassConstraint.new('Hello', :covariant)])
      'Hello'.should_not parse_to([Types::ClassConstraint.new('Hello', :contravariant)])
    end
  end
  ...
  describe "a complex class path" do
    it "is parsed into a single covariant constraint" do
      '::Hello::World::Is::Here'.should parse_to(
          [Types::ClassConstraint.new('::Hello::World::Is::Here', :covariant)])
    end
  end
  ...
  describe 'a class name followed by -' do
    it "is parsed into a contravariant class constraint" do
      'World::Is::Here-'.should parse_to(
          [Types::ClassConstraint.new('World::Is::Here', :contravariant)])
    end
  end
  ...
  describe 'a class name followed by =' do
    it "is parsed into a contravariant class constraint" do
      'World::Is::Here='.should parse_to(
          [Types::ClassConstraint.new('World::Is::Here', :invariant)])
    end
  end
  ...
  describe 'two constraints separated by =>' do
    it 'is parsed as a Hash<C1, C2>' do
      ['Symbol => String', 'Symbol=>String', 'Symbol  =>   String'].each do |input|
        input.should parse_to(
            [Types::GenericClassConstraint.new('Hash', :covariant,
                [[Types::ClassConstraint.new('Symbol', :covariant)],
    ...
    it 'allows variance constraints on the key and value types' do
      '::Hello::World==>Some::Constant-'.should parse_to(
          [Types::GenericClassConstraint.new('Hash', :covariant,
              [[Types::ClassConstraint.new('::Hello::World', :invariant)],
               [Types::ClassConstraint.new('Some::Constant', :contravariant)]])])
    end
  end
  ...
  describe 'a generic Array definition' do
    it 'is parsed as a GenericClassConstraint' do
      'Array<String>'.should parse_to(
          [Types::GenericClassConstraint.new('Array', :covariant,
              [[Types::ClassConstraint.new('String', :covariant)]])])
    end
  end
  ...
  describe 'a generic Hash definition' do
    it 'is parsed as a GenericClassConstraint' do
      'Hash- < Symbol=,   String  >'.should parse_to(
          [Types::GenericClassConstraint.new('Hash', :contravariant,
              [[Types::ClassConstraint.new('Symbol', :invariant)],
               [Types::ClassConstraint.new('String', :covariant)]])])
    end
  end
  ...
  describe 'an array generic shorthand' do
    it 'should parse as a covariant generic array constraint' do
      '[   String= ]'.should parse_to(
          [Types::GenericClassConstraint.new('Array', :covariant,
              [[Types::ClassConstraint.new('String', :invariant)]])])
    end
  end
    ...
    describe 'a simple tuple type' do
      it 'should parse to a TupleConstraint' do
        '(String- , _ , Symbol= => Fixnum)'.should parse_to(
            [Types::TupleConstraint.new(
              [[Types::ClassConstraint.new('String', :contravariant)],
               [Types::ClassConstraint.new('Object', :covariant)],
               [Types::GenericClassConstraint.new('Hash', :covariant,
                   [[Types::ClassConstraint.new('Symbol', :invariant)],
                    [Types::ClassConstraint.new('Fixnum', :covariant)]])]])])
      end
    end
  
    describe 'an empty tuple type' do
      it 'should parse to a (relatively useless) TupleConstraint' do
        '(  )'.should parse_to([Types::TupleConstraint.new([])])
      end
    end
  end
end